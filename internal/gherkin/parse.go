...
// step keyword, and as description text otherwise.
func (p *parser) handleDescriptionOrStep(line, col int, trimmed string) {
	...
	if !p.featureSet && looksLikeKeywordLine(trimmed) {
		// A keyword-shaped line before the Feature line is most likely a
		// non-English or misspelled keyword; report it instead of silently
		// absorbing it into a description.
